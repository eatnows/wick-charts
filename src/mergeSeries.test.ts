...
import { mergeSeriesPoints } from './mergeSeries';
...
describe('mergeSeriesPoints', () => {
  ...
  it('prefers the incoming point when times overlap', () => {
  ...
  it('handles a batch that fully overlaps already-loaded data', () => {
    ...
    const incoming = [candle(100, 1), candle(200, 2)]; // re-fetched overlap, no new points
    const merged = mergeSeriesPoints(existing, incoming);
  ...
  it('returns existing unchanged when incoming is empty', () => {
    const existing = [candle(100, 1)];
    expect(mergeSeriesPoints(existing, [])).toEqual(existing);
  });
});
